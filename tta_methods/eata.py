...
class EATA(nn.Module):
    ...
    def __init__(self, model, args):
        ...
        self.model_eata, self.optimizer, self.fishers = prepare_eata_model_and_optimizer(model, args)
        
        self.fisher_alpha = args.fisher_alpha # trade-off \beta for two losses (Eqn. 8) 
        ...
        self.e_margin = args.e_margin # hyper-parameter E_0 (Eqn. 3)
        self.d_margin = args.d_margin # hyper-parameter \epsilon for consine simlarity thresholding (Eqn. 5)
        ...
        self.model_state, self.optimizer_state = \
            copy_model_and_optimizer(self.model_eata, self.optimizer)
...
def prepare_eata_model_and_optimizer(model, args):
    ...
    if args.method == 'eta':
        ...
        fishers = compute_fishers(model, params, args)

    if args.optimizer == 'sgd':
        optimizer = torch.optim.SGD(params, lr=0.00025, momentum=0.9)
    
    check_model(model)
    return model, optimizer, fishers

def compute_fishers(model, params, args):
